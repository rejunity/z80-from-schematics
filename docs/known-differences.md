...
| **Real KC85 silicon, sync capture** (`make silicon_cycles`) | 50 classified opcodes | **50 OK (4 with /WAIT-attributed excess); 0 emu mismatches; 0 silicon system artifacts** | /WAIT (CH5) tracked per instruction; observed T-state excess where /WAIT is asserted is correctly attributed to Tw insertions |
| **Real KC85 silicon, 20 MHz async** (`make silicon_async`) | CPU clock + sub-T-state pin timing + 9-opcode cross-check | **CPU ≈ 1.767 MHz (0 WAIT samples in async capture — intrinsic period); M1/MREQ/RD/WR at spec sub-T-state offsets; 9 OK, 0 emu mismatches** | Independent cross-check; clock-period measurement is verified WAIT-free |
