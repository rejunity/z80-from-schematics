/* ============================================================================
 * z80_core.c - the Z80 CPU module: phase engine, bus-cycle setup, interrupt /
 * HALT / WAIT / BUSREQ acceptance, reset, and the public API.
 *
 * Mirrors rtl/z80_core.v. The z80_t struct is this module's registered state
 * (analogous to the Verilog module's internal regs); the public functions
 * below are the only external entry points.
 *
 * The combinational submodules called from here — z80_pla(), z80_alu(),
 * z80_timing() — live in their own .c files and match their Verilog
 * counterparts port-for-port. The instruction micro-sequencer
 * (z80_exec_step) lives in z80_seq.c (mirrors rtl/z80_seq.v); we call into
 * it once per completed M-cycle. Engine-internal cross-module declarations
 * (inline accessors + the three core<->seq prototypes) live in
 * z80_internal.h.
 *
 * File contents in order:
 *   1. Bus-cycle setup: z80_busop_base_len() (static), z80_start_mcycle()
 *      and z80_start_m1() — the latter two called by the sequencer.
 *   2. Per-phase engine + reset / init — the public API surface
 *      (z80_init, z80_reset, z80_phase_step, z80_set_pc) plus the
 *      interrupt/HALT/BUSREQ acceptance helpers.
 * ==========================================================================*/
#include <string.h>
#include "z80_internal.h"

/* =========================================================================
 * 1. Bus-cycle setup (formerly z80_bus.c, then Section 2 of z80_core.c)
 * =======================================================================*/
...
/* =========================================================================
 * 2. Per-phase engine, reset, init — public API
 *
 ...
 *      end-of-M-cycle dispatch into z80_exec_step over in z80_seq.c),
 *   2) samples inputs (read-data latch / wait) for the phase about to drive,
 *   3) drives the external pins for that phase via z80_timing().
