...
#include "refs/suzukiplan_z80.hpp"   // declares class Z80 (no rename needed)
...
static uint8_t  PMEM[65536];    /* suzukiPlan */
...
int main(int argc, char** argv) {
    ...
    /* suzukiplan/z80 (4th oracle, MAME-style industry reference) */
    Z80 P(
    ...
    for (; n < cap; n++) {
        ...
        P.execute(1);                     /* suzukiplan: smallest budget => 1 instr */

        /* compare main regs (PC, AF, BC, DE, HL, IX, IY, SP) across all four */
        ...
        /* suzukiplan: registers in .reg.pair.* and .reg.PC etc. */
        ...
        bool diverge = (mpc2 != spc || mpc2 != cpc || mpc2 != ppc ||
                        ...
                        mix != six || mix != C.ix || mix != pix ||
                        miy != siy || miy != C.iy || miy != piy ||
                        msp != ssp || msp != C.sp || msp != psp);
        if (diverge) {
    ...
    printf("quad-lockstep clean: %lld instructions in %.2f s (%.2f Minstr/s, four emulators)\n",
        n, secs, n/secs/1e6);
    free(M);
    return 0;
}
