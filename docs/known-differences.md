...
| **Real KC85 silicon, sync capture** (`make silicon_cycles`) | 50 classified opcodes | **45 emu = spec = silicon; 0 emu mismatches** | 5 opcodes the KC85 hardware injects extra WAIT/interrupt cycles into — system-level, not Z80-level |
| **Real KC85 silicon, 20 MHz async** (`make silicon_async`) | 9 opcodes in window + CPU clock + sub-T-state pin timing | **CPU ≈ 1.767 MHz, M1/MREQ/RD/WR at spec sub-T-state offsets, 8 OK, 0 emu mismatches** | Independent cross-check of `silicon_cycles` via re-sampling the async stream at CLK falling edges |
