...
## Seeded-random programs (`prog_rnd_*.hex`)

Hand-written programs only cover patterns the author thought to write. To
catch the patterns nobody thought of, the test corpus includes a handful
of programs generated by [`scripts/gen_random_trace_progs.py`](../../scripts/gen_random_trace_progs.py) —
deterministic from a numeric seed (same seed always produces the same
bytes), drawing opcodes from a curated "safe" pool (no JP / JR / CALL /
RET / HALT — those would jump or stop in undefined ways inside a
200-phase window).

The opcode pool covers the LD-r,r' grid, ALU on register / immediate,
INC / DEC r, INC / DEC rp, ADD HL,rp, rotate / shift, EX / EXX, CB
register ops, and a curated subset of ED instructions (IN r,(C) /
OUT (C),r, ADC / SBC HL,rp, LD (nn),rp / LD rp,(nn), NEG, IM 0/1/2,
LD I,A / LD R,A / LD A,I / LD A,R, RRD / RLD).

Current set: `prog_rnd_01.hex` ... `prog_rnd_04.hex`, 48 bytes each,
seeds 1..4. To add more:

    scripts/gen_random_trace_progs.py 5 48 tests/traces/prog_rnd_05.hex

Then re-run `make compare`, `make perfectz80`, and `make perfectz80_rtl`
— the new program is auto-discovered (the compare script globs
`prog_rnd_*.hex`).

The random programs surfaced one real loader-parity bug in
`scripts/perfectz80_runner.c` (it didn't skip `#` / `//` comment lines —
fixed) and otherwise pass all three gates: C↔iverilog↔Verilator parity,
C-vs-perfectz80, and iverilog-RTL-vs-perfectz80.
...
**Current state (2026-06-20)**: 8 / 12 PASS clean on **both C model
and iverilog RTL** after Steps 0-5 of the silicon-faithful sweep +
the reset-window mask in `compare_signal_timing.py`. Documented in
[docs/perfect-branch.md](../../docs/perfect-branch.md):

  - PASS (both C and RTL): `prog9_inta_im1`, `prog12_inta_im2`,
    `prog15_busreq_m1` (Step 5), `prog16_ei_delay`,
    `prog17_reset` (Step 4 — both implementations detect reset
    immediately; harness masks pz80's NMOS-process recognition delay),
    `prog18_di_then_int`, `prog19_nmi_in_int`, `prog20_block_int`
  - 1 ctrl-pin diff: `prog10_halt_nmi` (sub-T-state HALT timing
    during NMI ack)
  - Informational diffs in two classes (per-program breakdown in
    [docs/perfect-branch.md](../../docs/perfect-branch.md)):
    `prog11_wait_mem` (C 142 / RTL 161), `prog13_halt_int` (144),
    `prog14_wait_io` (147)

See [docs/known-differences.md](../../docs/known-differences.md) row 14
for the diff list summary and [docs/perfect-branch.md](../../docs/perfect-branch.md)
for the per-pin compatibility table + "Remaining differences vs
perfectz80" mechanism-and-resolution table.
