...
module z80_core (
    ...
    always @* begin
        ...
        case (exec_w)
            ...
            `EXEC_RRD:       begin alu_md = `FLAG_RRD; alu_b = rbyte; end
            `EXEC_RLD:       begin alu_md = `FLAG_RLD; alu_b = rbyte; end
            default:             alu_b = 8'h00;
        endcase
    end
    ...
    z80_seq u_seq (
        ...
        .ctl_tmpl_we_alu_mem(seq_tmpl_we_alu_mem)
    );
    ...
    always @* begin
        ...
        // advance phase
            ...
            if ((t_state + 4'd1) > m_len) begin
                ...
                `EXEC_LD_RP_NN: ;  /* migrated to z80_seq (ctl_rp_set_nn) */
                ...
                `EXEC_LD_I_A, `EXEC_LD_R_A, `EXEC_LD_A_IR: ;  /* migrated to z80_seq */
                `EXEC_LD_NNA_RP, `EXEC_LD_RP_NNA: ;  /* migrated to z80_seq */
                `EXEC_IN_C, `EXEC_OUT_C, `EXEC_RRD, `EXEC_RLD: ;  /* migrated to z80_seq via ALU FLAG_IN/RRD/RLD */
                ...
                if (seq_active) begin
                    ...
                    if (seq_tmpl_we_alu_mem) tmpl_n  = alu_mem_byte;
                    ...
                    if (seq_load_a_ir) begin
                        // A = aux[0] ? R : I; F per LD A,I/R formula
                        rf_n[`RFP_AF][15:8] = aux_w[0] ? reg_r : reg_i;
                        rf_n[`RFP_AF][7:0]  = (F_cur & `Z80_CF)
                                            | ((aux_w[0] ? reg_r[7]      : reg_i[7])      ? `Z80_SF : 8'h0)
                                            | (((aux_w[0] ? reg_r        : reg_i) == 8'h0)? `Z80_ZF : 8'h0)
                                            | ((aux_w[0] ? reg_r         : reg_i) & (`Z80_YF | `Z80_XF))
                                            | (iff2                                      ? `Z80_PF : 8'h0);
                    end
                    ...
                    if (seq_rp_set_nn) rf_n[rp_sel_w] = {rbyte, tmpl};
                    if (seq_rp_inc)    rf_n[rp_sel_w] = rf[rp_sel_w] + 16'd1;
                    if (seq_rp_dec)    rf_n[rp_sel_w] = rf[rp_sel_w] - 16'd1;
                    if (seq_sp_set_hl) rf_n[`RFP_SP]  = rf[hlp];
                end
