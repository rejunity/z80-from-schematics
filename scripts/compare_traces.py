...
def compare_one(prog, phases):
    # programs named *nmi* get an NMI pulse at phase 30 in both harnesses
    nmi = ["30"] if "nmi" in os.path.basename(prog) else []
    c = trace_lines(run([TRACEGEN, prog, phases] + nmi))
    vargs = [VVP, f"+prog={prog}", f"+phases={phases}"] + ([f"+nmi={nmi[0]}"] if nmi else [])
    v = trace_lines(run(vargs))
    n = min(len(c), len(v))
